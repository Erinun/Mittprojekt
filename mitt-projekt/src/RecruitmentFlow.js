      ...
      subSteps: [
        {
          ...
          infoList: [
            ...
            "Läs på om rollen, organisationen, enheten och tittar eventuellt igenom tidigare material i rekryteringssystemet",
...
"HR-konsult skickar alla bokningar i Outlook till berörda parter och efter överenskommelse mejlar tidplanen till rekryterande chef och sparar på ärendet i rekryteringssystemet.",
    ...
    <div className="flow-container">
      {steps.map((step) => (
        ...
        >
          {/* Huvudsteget */}
          <div
            className={`step ${activeStep === step.id ? 'active' : 'inactive'}`}
              ...
              <div className="sub-steps">
                {step.subSteps.map((sub, idx) => (
                  <div key={idx} className="sub-step-container">
                    <button onClick={() => handleSubStepClick(sub)}>
                      {sub.title}
                    </button>
                    {selectedSubStep?.title === sub.title && (
                      <div className="substep-info">
                        <h4>{sub.title}</h4>
                        {/* Visa punktlista om infoList finns */}
                        {sub.infoList && (
                          <ul>
